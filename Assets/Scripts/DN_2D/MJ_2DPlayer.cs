...
public class MJ_2DPlayer : MonoBehaviour
{
    [Header("이동 설정")]
    ...
    [Header("애니메이터")]
    [SerializeField] private MJ2DBoyAnimController Animator_2DPlayer;
    ...
    void Update()
    {
        ...
        ChangePlayerState(isMoving ? AnimState.Walk : AnimState.Idle);
    ...
    private void ChangePlayerState(AnimState newState)
    {
        // 이런 곳에 UI나 플레이어의 별도 처리를 넣어줄 수도 있다


        // 우선 애니메이션만 바꿔 봅시다
        Animator_2DPlayer.SetAnimState(newState);
    }
    ...
    //private void HandleGameOver(string msg) // 성취도 평가때 사용
    //{
    ...
    public bool CheckSkillUseable(bool isShowMsg = true)
    {
        if (_isSkillUsing == true)
        {
            if (isShowMsg == true)
            {
                DaniTechUIManager.Instance.OpenSimplePopup("스킬이 이미 사용 중 입니다.");
    ...
    public void UseNormalAttack()
    {
        ...
        ChangePlayerState(AnimState.Atk);
